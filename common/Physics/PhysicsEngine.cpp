...
{
    //Position of blocks is held in block datastructure which hasn't been implemented yet.
    //std::sort(blockList.begin(), blockList.end(), [](){}); //sort block list based on lowest height 
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, blockList.size()), [&](const tbb::blocked_range<int>& r)
    {
