...
public:
	...
	MaterialContainer(std::string name, std::vector<T> contents,double freq,double smoothness)
		:name(name),contents(contents),freq(freq),smoothness(smoothness)
	{
		setContentFreq();
	}
	MaterialContainer(const MaterialContainer & MC)
		:name(MC.name),contents(MC.contents),freq(MC.freq),smoothness(MC.smoothness),contentFreq(MC.contentFreq){}
...
public:
	...
	Ground(std::string name, std::vector<Material> contents,double freq,double smoothness)
...
public:
	...
	Biome(std::string name, std::vector<Ground> contents,double freq,double smoothness)
		:MaterialContainer(name,contents,freq,smoothness){}
};
...
{
public:
	Biosphere(std::string name, std::vector<Biome> contents,double freq,double smoothness)
		:MaterialContainer(name,contents,freq,smoothness){}
