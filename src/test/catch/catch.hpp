...
inline Clara::CommandLine<ConfigData> makeCommandLineParser()
{
  ...
  cli["-w"]["--warn"].describe("enable warnings").bind(
  ...
  cli["--order"].describe("test case order (defaults to decl)").bind(
    &setOrder, "decl|lex|rand");

  cli["--rng-seed"].describe("set a specific seed for random numbers").bind(
    &setRngSeed, "'time'|number");
...
inline std::size_t listTags(Config const& config)
{
  ...
  for (
    std::map<std::string, TagInfo>::const_iterator countIt = tagCounts.begin(),
...
namespace
{
  ...
  IColourImpl* platformColourInstance()
  {
    return isatty(STDOUT_FILENO) ? PosixColourImpl::instance()
...
#define INTERNAL_CATCH_REGISTER_LEGACY_REPORTER(name, reporterType)            \
  ...
  {                                                                            \
    Catch::LegacyReporterRegistrar<                                            \
      reporterType> catch_internal_RegistrarFor##reporterType(name);           \
  }
...
public:
  ...
  void writeGroup(TestGroupNode const& groupNode, double suiteTime)
  {
    ...
    xml.scopedElement("system-out").writeText(
      trim(stdOutForSuite.str()), false);
    xml.scopedElement("system-err").writeText(
      trim(stdErrForSuite.str()), false);
  }
